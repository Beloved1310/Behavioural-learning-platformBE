    ...
    async findByEmailWithPassword(email) {
        ...
        if (process.env.NODE_ENV === 'development' && !user) {
            ...
            if (userWithoutPassword) {
                logger_1.logger.error('User exists but password field not retrieved', undefined, {
        ...
        try {
            ...
            if (leanUser) {
                ...
                if (user) {
                    ...
                    if (userToken === normalizedToken) {
                        ...
                        logger_1.logger.warn('Token mismatch after fetch', {
            ...
            if (process.env.NODE_ENV === 'development') {
                ...
                const debugUser = await this.model
                    ...
                    .lean();
                if (debugUser) {
                    logger_1.logger.error('User found but field selection failed. Token exists', undefined, {
                        userId: debugUser._id.toString(),
                        email: debugUser.email,
                        isVerified: debugUser.isVerified,
                        hasToken: !!debugUser.verificationToken,
                        tokenInDB: debugUser.verificationToken
                            ? debugUser.verificationToken.substring(0, 20) + '...'
                            : 'null',
                    });
                    ...
                    if (userById && userById.verificationToken) {
                        logger_1.logger.debug('Successfully retrieved token via findById', {
                    ...
                    if (anyUserWithToken) {
                        ...
                        logger_1.logger.debug('Users with verification tokens exist, but token mismatch', {
                        ...
                        logger_1.logger.debug('No users found with verification tokens', {
                        ...
                        const recentUser = await this.model
                            ...
                            .lean();
                        if (recentUser) {
                            logger_1.logger.debug('Most recent unverified user', {
            ...
            logger_1.logger.error('Error finding user by verification token', error);
            return null;
        }
    }
    /**
