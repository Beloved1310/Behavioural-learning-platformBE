...
class SessionsController {
    ...
    static async getUserSessions(req, res) {
        try {
            ...
            throw new errorHandler_1.AppError('Failed to fetch sessions', 500);
        }
    }
    // Get session by ID
    ...
    static async getSessionStats(req, res) {
        try {
            ...
            const scheduledSessions = await models_1.Session.countDocuments({ ...query, status: types_1.SessionStatus.SCHEDULED });
            const completedSessions = await models_1.Session.countDocuments({ ...query, status: types_1.SessionStatus.COMPLETED });
            const missedSessions = await models_1.Session.countDocuments({ ...query, status: types_1.SessionStatus.MISSED });
            const cancelledSessions = await models_1.Session.countDocuments({ ...query, status: types_1.SessionStatus.CANCELLED });
            ...
            throw new errorHandler_1.AppError('Failed to fetch session statistics', 500);
        }
    }
    ...
    static async searchTutors(req, res) {
        try {
            ...
            throw new errorHandler_1.AppError('Failed to search tutors', 500);
        }
    }
    ...
    static async bookTutorSession(req, res) {
        try {
            const userId = req.user.id;
