...
class PaymentController {
    ...
    static async getPaymentHistory(req, res) {
        try {
            ...
            const err = new Error('Failed to fetch payment history');
            err.statusCode = 500;
            throw err;
        }
    }
    ...
    static async getPaymentStats(req, res) {
        try {
            ...
            const err = new Error('Failed to fetch payment statistics');
            err.statusCode = 500;
            throw err;
        }
    }
    ...
    static async createPaymentIntent(req, res) {
        try {
            ...
            // Create payment intent
            const paymentIntent = await stripe_1.StripeService.createPaymentIntent(Math.round(amount * 100), // Convert to pence
            currency, stripeCustomerId, description || 'Payment');
            ...
            throw new errorHandler_1.AppError(error.message || 'Failed to create payment intent', 500);
        }
    }
    // Confirm payment
    static async confirmPayment(req, res) {
        try {
            ...
            const payment = await Payment_1.Payment.findOne({ stripePaymentIntentId: paymentIntentId });
            if (!payment) {
            ...
            throw new errorHandler_1.AppError(error.message || 'Failed to confirm payment', 500);
        }
    }
    ...
    static async getSubscription(req, res) {
        try {
            const userId = req.user?._id;
            const subscription = await Subscription_1.default.findOne({ userId, status: 'active' });
            if (!subscription) {
            ...
            throw new errorHandler_1.AppError('Failed to fetch subscription', 500);
        }
    }
    // Create subscription
    static async createSubscription(req, res) {
        try {
            ...
            throw new errorHandler_1.AppError(error.message || 'Failed to create subscription', 500);
        }
    }
    // Update subscription
    static async updateSubscription(req, res) {
        try {
            ...
            throw new errorHandler_1.AppError(error.message || 'Failed to update subscription', 500);
        }
    }
    // Cancel subscription
    static async cancelSubscription(req, res) {
        try {
            ...
            throw new errorHandler_1.AppError(error.message || 'Failed to cancel subscription', 500);
        }
    }
    // Get payment methods
    static async getPaymentMethods(req, res) {
        try {
            const userId = req.user?._id;
            const paymentMethods = await PaymentMethod_1.default.find({ userId, isActive: true }).sort({
                isDefault: -1,
                createdAt: -1
            });
            ...
            throw new errorHandler_1.AppError('Failed to fetch payment methods', 500);
        }
    }
    // Add payment method
    static async addPaymentMethod(req, res) {
        try {
            ...
            if (isDefault) {
                await PaymentMethod_1.default.updateMany({ userId, _id: { $ne: null } }, { isDefault: false });
            }
            const paymentMethod = await PaymentMethod_1.default.create(paymentMethodData);
            if (isDefault) {
            ...
            throw new errorHandler_1.AppError(error.message || 'Failed to add payment method', 500);
        }
    }
    ...
    static async deletePaymentMethod(req, res) {
        try {
            const userId = req.user?._id;
            const { id } = req.params;
            const paymentMethod = await PaymentMethod_1.default.findOne({ _id: id, userId });
            if (!paymentMethod) {
                throw new errorHandler_1.AppError('Payment method not found', 404);
            }
            // Detach from Stripe
            if (paymentMethod.stripePaymentMethodId) {
                await stripe_1.StripeService.detachPaymentMethod(paymentMethod.stripePaymentMethodId);
            }
            ...
            throw new errorHandler_1.AppError(error.message || 'Failed to delete payment method', 500);
        }
    }
    ...
    static async setDefaultPaymentMethod(req, res) {
        try {
            ...
            if (paymentMethod.stripeCustomerId && paymentMethod.stripePaymentMethodId) {
                await stripe_1.StripeService.setDefaultPaymentMethod(paymentMethod.stripeCustomerId, paymentMethod.stripePaymentMethodId);
            }
            res.json({
                success: true,
            ...
            throw new errorHandler_1.AppError(error.message || 'Failed to set default payment method', 500);
        }
    }
    // Request refund
    static async requestRefund(req, res) {
        try {
            ...
            const payment = await Payment_1.Payment.findOne({ _id: paymentId, userId });
            if (!payment) {
                throw new errorHandler_1.AppError('Payment not found', 404);
            }
            ...
            const existingRefund = await RefundRequest_1.default.findOne({ paymentId });
            if (existingRefund) {
                throw new errorHandler_1.AppError('Refund already requested for this payment', 400);
            }
            // Create refund request
            const refundRequest = await RefundRequest_1.default.create({
                paymentId,
                userId,
            ...
            throw new errorHandler_1.AppError(error.message || 'Failed to request refund', 500);
        }
    }
    // Get refund requests
    static async getRefundRequests(req, res) {
        try {
            const userId = req.user?._id;
            const refunds = await RefundRequest_1.default.find({ userId })
            ...
            throw new errorHandler_1.AppError('Failed to fetch refund requests', 500);
        }
    }
    ...
    static async processRefund(req, res) {
        try {
            ...
            const refundRequest = await RefundRequest_1.default.findById(id).populate('paymentId');
            if (!refundRequest) {
            ...
            throw new errorHandler_1.AppError(error.message || 'Failed to process refund', 500);
        }
    }
}
