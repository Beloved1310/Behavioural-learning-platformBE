...
exports.commonFields = {
    ...
    dateOfBirth: joi_1.default.date().min('1900-01-01').max('now').iso().messages({
    ...
    limit: joi_1.default.number().integer().min(1).max(1000).default(20).messages({
    ...
    subscriptionTier: joi_1.default.string()
        .valid(...Object.values(types_1.SubscriptionTier))
        .messages({
        'any.only': `Subscription tier must be one of: ${Object.values(types_1.SubscriptionTier).join(', ')}`,
    }),
    sessionStatus: joi_1.default.string()
        .valid(...Object.values(types_1.SessionStatus))
        .messages({
        'any.only': `Session status must be one of: ${Object.values(types_1.SessionStatus).join(', ')}`,
    }),
    // paymentStatus - Removed from MVP
    ...
    messageType: joi_1.default.string()
        .valid(...Object.values(types_1.MessageType))
        .messages({
        'any.only': `Message type must be one of: ${Object.values(types_1.MessageType).join(', ')}`,
    }),
    badgeType: joi_1.default.string()
        .valid(...Object.values(types_1.BadgeType))
        .messages({
    ...
    subject: joi_1.default.string()
        .trim()
        .min(1) // Reject empty strings after trimming
        .valid('Mathematics', 'English', 'Science', 'Physics', 'Chemistry', 'Biology', 'History', 'Geography', 'Computer Science', 'Art', 'Music', 'Spanish', 'French', 'German')
        .messages({
    ...
    sortField: joi_1.default.string()
        .valid('createdAt', 'updatedAt', 'name', 'email', 'rating', 'price', 'date', 'score')
        .default('createdAt')
        .messages({
