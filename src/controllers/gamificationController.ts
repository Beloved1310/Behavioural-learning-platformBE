...
import { Quiz, QuizAttempt, Badge, UserBadge, UserProgress, User } from '../models';
import { AppError } from '../middleware/errorHandler';

export class GamificationController {
  // Get all quizzes with optional filters
  static async getQuizzes(req: AuthenticatedRequest, res: Response) {
    try {
  ...
  static async getQuizById(req: AuthenticatedRequest, res: Response) {
    try {
      const { id } = req.params;
  ...
  // Submit quiz attempt
  static async submitQuizAttempt(req: AuthenticatedRequest, res: Response) {
    try {
  ...
  static async getRecentAttempts(req: AuthenticatedRequest, res: Response) {
    try {
      const userId = req.user!.id;
      ...
      throw new AppError('Failed to fetch recent attempts', 500);
    }
  }
  ...
  static async getUserProfile(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const user = await User.findById(userId).select('totalPoints streakCount lastLoginAt');
  ...
  static async getUserProgress(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const progress = await UserProgress.find({ userId }).sort({ subject: 1 });
  ...
  static async getAvailableBadges(req: AuthenticatedRequest, res: Response) {
    try {
      const badges = await Badge.find({ isActive: true }).sort({ rarity: 1, pointsReward: 1 });
  ...
  private static async checkBadgeEligibility(userId: string, quizPercentage: number, subject: string) {
    try {
      const user = await User.findById(userId);
      if (!user) return [];
      ...
      for (const badge of allBadges) {
        ...
        switch (badge.criteria.type) {
          ...
          case 'quiz_count':
            const quizCount = await QuizAttempt.countDocuments({ studentId: userId });
