...
export const updateProfileSchema = {
  body: Joi.object({
    ...
    academicGoals: Joi.alternatives().try(commonFields.academicGoals, Joi.string().allow('')),
    // Tutor-specific fields
    subjects: Joi.alternatives().try(commonFields.subjects, Joi.string().allow('')),
    bio: commonFields.mediumText.allow(''),
    qualifications: Joi.alternatives().try(commonFields.qualifications, Joi.string().allow('')),
  })
    .min(1)
    .messages({
      'object.min': 'At least one field must be provided for update',
