...
userBadgeSchema.statics.getUserBadges = function (userId: string) {
  return this.find({ userId }).populate('badge').sort({ earnedAt: -1 });
};

// Static method to check if user has badge
userBadgeSchema.statics.hasUserEarnedBadge = function (userId: string, badgeId: string) {
  return this.exists({ userId, badgeId });
};

// Static method to get badge leaderboard
userBadgeSchema.statics.getBadgeLeaderboard = function (limit: number = 10) {
