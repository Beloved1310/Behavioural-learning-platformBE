...
progressReportSchema.statics.getLatestReport = function (
  studentId: string,
...
progressReportSchema.statics.getReportHistory = function (
  studentId: string,
  period: 'weekly' | 'monthly',
  limit: number = 10
) {
  return this.find({ studentId, period }).sort({ generatedAt: -1 }).limit(limit);
};

// Static method to generate insights based on data
progressReportSchema.statics.generateInsights = function (reportData: any) {
  const insights = [];
  ...
  if (reportData.streakDays > 7) {
    insights.push(
      `Great consistency! You've maintained a ${reportData.streakDays}-day learning streak.`
    );
  }
...
progressReportSchema.statics.generateRecommendations = function (reportData: any) {
