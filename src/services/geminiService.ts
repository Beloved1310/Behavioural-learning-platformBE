...
export class GeminiService {
  ...
  private static reflectionModel: GenerativeModel | null = null;
  private static nudgeModel: GenerativeModel | null = null;
  ...
  private static extractJsonFromText(text: string): string[] {
    try {
      ...
      logger.error('JSON extraction error', error, { text: text.substring(0, 100) });
      throw error;
    }
  }
...
Return ONLY a JSON array of strings, like: ["prompt 1", "prompt 2", "prompt 3"]`;
      ...
      try {
        ...
        logger.error('Failed to parse reflection prompts JSON', parseError);
      }
...
Return ONLY a JSON array of strings, like: ["nudge 1", "nudge 2", "nudge 3"]`;
      ...
      try {
        ...
        logger.error('Failed to parse nudges JSON', parseError);
      }
   ...
   */
  private static getDefaultReflectionPrompts(): string[] {
    return [
   ...
   */
  private static getDefaultNudges(): string[] {
    return [
